...
import ArtworkWithGuide from '@/components/ArtworkWithGuide';
...
export async function generateMetadata({ params }) {
  ...
  return {
    ...
    openGraph: { title: `${artwork.title} | Artivoya`, description: artwork.description },
  };
}
...
export default async function GalleryDetailPage({ params }) {
  ...
  const relatedArtworks = getGalleryBySeries(artwork.series).filter((item) => item.id !== artwork.id);

  return (
    <div className="artwork-detail">
      <div className="container">
        <div className="artwork-detail-grid">
          {/* 이미지 + 가이드를 하나의 클라이언트 래퍼로 */}
          {artwork.image ? (
            <ArtworkWithGuide imageUrl={artwork.image} slug={slug}>
              <Image
                src={artwork.image}
                alt={artwork.title}
                fill
          ...
          <div className="artwork-info">
            {seriesData && (
              <Link href={`/${seriesData.slug}`} className="artwork-series-badge" style={{ background: seriesData.primaryColor }}>
            ...
            <div style={{ display: 'flex', gap: 'var(--space-md)', flexWrap: 'wrap' }}>
              {seriesData && (
                <Link href={`/${seriesData.slug}`} className="btn btn-series" style={{ '--series-color': seriesData.primaryColor }}>
